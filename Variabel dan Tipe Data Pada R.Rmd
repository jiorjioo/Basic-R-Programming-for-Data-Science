...
a = 30
print(a)
c = "Ini bahasa R, masih agak asing tpi semoga cepet familiar"
print(c)
```
...
gram_ke_kilogram = 1*0.001
tiga_gram_ke_kilogram = gram_ke_kilogram*300
print(tiga_gram_ke_kilogram)
...
#bakal error karena penamaan variabel pake spasi :")

banyakHari = 30
print(banyakHari)
print("bahasa R syntax-nya sederhana")
```
...
#oh jadi vektor semacam array.. cara nulisnya: c(isi vektor)
```

kita juga bisa membuat rangkaian data dengan operator titik dua. 
Sebagai contoh :
```{r}
c(1:5) #1:5 --> satu sampai lima= 1 2 3 4 5
c(1:10:2) #error, ternyata gabisa disamain kayak python yang dia bisa lompat2 angkanya
c(1:3)
c(5:1) #hah bisa ngitung mundur
```
...
y = c("apa", 8.0, 10.000) #menyusahkan diri sendiri dgn kerja dua kali wkwkw :P
print(y)
...
Sebagi contoh :
```{r}
summary(c(1:5)) #edan outputnya.

x= c(10:1)
print(x)
summary(x) #gilaaaaa so cool!
...
A <- matrix(c(1,3,5,7,9,11), #Vektor elemen matriks
            3,               #Jumlah baris
            2)               #Jumlah kolom
A
B = c(2,4,6,8)
...
*Matriks Simetri* matriks simetri --> jika matriks A = matriks A transpose

```{r}
#Mendefinisikan matriks simetri K
K <- matrix(c(2,1,5,1,3,4,5,4,-2),3,3) #3,3 di belakang maknanya baris-kolom
print(K)
```
```{r}
#Transpose matriks K
...
info_buah <- data.frame(buah, jumlah_buah) #syntax= data.frame(yang mau ditampilkan)
...
#Tampilkan DataFrame
info_buah 
#HAH KAGET BGT OUTPUTNYA KOK KEREN, RAPI...
```
```{r}
#mau nyoba sendiri
hewan_buas = c("hiu", "harimau", "singa", "ular")
jumlah = c(2, 3, 3, 5)
info_hewan_buas = data.frame(hewan_buas, jumlah)
print(info_hewan_buas)

  #nambah variabel
habitat = c("air", "darat", "darat", "darat")
info_hewan_buas = data.frame(info_hewan_buas, habitat)
...
#Menampilkan kolom rasa | penggunaan tanda $ biar lebih spesifik
info_buah$rasa

#Menampilkan kolom buah | penggunaan tanda $ biar lebih spesifik
info_buah$buah
```
...
Jadi levels diurutkan bukan urutan mundur tapi tergantung urutan alfabet.
...
faktor.warung= factor(c("soto","bakso","sate","ayam","pangsit","rujak"))

attributes(faktor.hari)#[.]
attributes(faktor.warung)
```
...
faktor.warung= factor(c("soto","bakso","sate","ayam","pangsit","rujak")) 
levels(faktor.warung) #tampilan output akan urut berdasar alfabet
class(faktor.warung)
```
...
#jadi.. as.integer tu dah diurutin berdasar alfabet, terus hasil outputnya adl index-indexnya factor, bedanya disini index yang punya substansi yang sama ga diilangin (dilebur) alias ttp ditulis. 

faktor.warung= factor(c("soto","bakso","soto","ayam","bakso","rujak", "rujak")) 
levels(faktor.warung)
as.integer(faktor.warung)

#variasinya ada 4: soto, bakso, ayam, rujak
#levels= ayam, bakso, rujak, soto 
#as.integer= soto, bakso, soto, ayam, bakso, rujak, rujak
```
...
faktor.warung= factor(c("soto","bakso","soto","ayam","bakso","rujak", "rujak")) 
levels(faktor.warung)[3] <- "gado-gado" #(angka index disini yang dah diurutin dari levels)
faktor.warung
```
...
factor.sks#[.2.]
as.integer(factor.sks) #diurutin berdasar urutan angka/ alfabet
```
...
factor.merk <- factor(c("Samsung", "Apple", NA, NaN, "Asus", "Acer", NULL))
length(factor.merk)
```
...
factor(c(1,5,6,3,2,4,9), levels = c(9,6,5,4,3,2,1))
```
...
list_acak <- list("psds", 2.0, "matematika")
